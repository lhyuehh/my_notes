...
| 周期 | RankIC均值 | RankIC胜率 | 年化ICIR (RankIC) |
| ---- | ---------- | ---------- | ---- |
| 1D | 0.0391 | 64.39% | 4.4848 |
| 5D | 0.0582 | 67.28% | 2.7381 |
| 10D | 0.0701 | 70.20% | 2.2409 |
| 20D | 0.0867 | 72.53% | 1.8664 |

**指标分析**：
本分析使用行业标准开源框架 **Alphalens** 计算指标。因子在各个周期均展现出极强的正向预测能力。其中，**最优预测周期为 1D**，说明因子在多日维度的预测稳定性强，有效性高。
...
**分析**：经 Alphalens 框架测试，10分层展现出**严格单调性**。最高分层（第10组）平均收益最高，而最低分层（第1组）收益最低，完美验证了因子优秀的横截面选股能力。
...
- **多头组合年化收益率**：29.89%
- **全市场基准年化收益率**：5.01%
- **超额收益率（相对全市场基准）**：5069.64%
- **最大回撤**：64.38%
- **回测合规性说明**：
  已扣除双边千分之二（0.2%）的交易手续费及滑点。
  模拟真实交易环境，在T日收盘后计算因子值，于 **T+1日** 生成目标仓位，并以 **T+1日到T+2日** 的真实收益率进行结算。
  严格遵守涨跌停规则：当日涨停的股票不可买入，当日跌停的股票限制卖出（目标仓位不得低于自然漂移仓位）。
  严格剔除了ST股及上市不足60日的次新股。
  调仓频率：日频调仓。
